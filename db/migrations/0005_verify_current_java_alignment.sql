...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='message_warnings' AND column_name='topic_id') THEN
    EXECUTE $sql$
      ...
      WHERE topic IS NULL OR author IS NULL OR message IS NULL
    $sql$;
  END IF;
END$$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='topics' AND column_name='warning_counter') THEN
    EXECUTE 'UPDATE topics SET open_warnings = warning_counter WHERE open_warnings = 0 AND warning_counter IS NOT NULL';
  END IF;
END$$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reactions_log' AND column_name='userid')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reactions_log' AND column_name='msgid') THEN
    EXECUTE $sql$
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_invites' AND column_name='created_at') THEN
    EXECUTE $sql$
      UPDATE user_invites
