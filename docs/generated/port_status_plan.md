...
#### P2 — крупные отсутствующие фичи (реализовать с нуля, ничего не сделано)
1. **Загрузка изображения в галерею при создании темы** — раздел "Галерея" не может выполнять свою основную функцию.
2. **Создание/редактирование опросов** — нет формы с вариантами/multiselect при создании темы, нет вставки в `polls`/`polls_variants`; из-за этого голосование (уже исправленное ранее) недостижимо через обычный интерфейс создания темы.
3. `/view-all.jsp` — очередь премодерации для модераторов/корректоров.
4. Инлайн-виджет реакций на странице темы/комментария (сейчас реакции работают только через отдельный `/reactions`).
5. `/people/{nick}/reactions/{mode}` (режим "реакции на мои посты") и `/tags?term=` автокомплит.
6. Полноценные archive-страницы с индексом по годам/месяцам (сейчас плоский список).
...
- Пагинация уведомлений не группирует подряд идущие события одного типа/темы (сейчас неважно, т.к. WARNING/REACTION-события не создаются вовсе — см. п.5).
