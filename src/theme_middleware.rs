...
fn theme_view(style: &str) -> ThemeView<'_> {
    match style {
        "tango" => ThemeView { style, color_mode: Some("dark"), stylesheet: "/tango/combined.css" },
        "tango-light" => ThemeView { style, color_mode: Some("light"), stylesheet: "/tango/combined.css" },
        "tango-auto" => ThemeView { style, color_mode: Some("auto"), stylesheet: "/tango/combined.css" },
        "black" => ThemeView { style, color_mode: None, stylesheet: "/black/combined.css" },
        "white2" => ThemeView { style, color_mode: None, stylesheet: "/white2/combined.css" },
        "waltz" => ThemeView { style, color_mode: None, stylesheet: "/waltz/combined.css" },
        "zomg_ponies" => ThemeView { style, color_mode: None, stylesheet: "/zomg_ponies/combined.css" },
    ...
    if let Some(session) = jar.get("lor_session") {
        if let Some(user_id) = crate::auth::verify_session(session.value(), &state.config.cookie_secret) {
            ...
            if let Some((style, nick)) = profile {
                return (
                    style.filter(|value| is_style(value)).unwrap_or_else(|| DEFAULT_STYLE.to_string()),
                    Some(nick),
                );
            }
        }
...
fn waltz_header(nick: Option<&str>) -> String {
    ...
    let guest = if nick.is_none() { login_block(None, false) } else { String::new() };
    format!(r#"<header id="hd"><div id="topProfile">{top_profile}</div><span id="sitetitle"><a href="/">LINUX.ORG.RU</a></span><nav class="menu"><div id="loginGreating">{guest}</div><ul><li><a href="/news/">Новости</a></li><li><a href="/gallery/">Галерея</a></li><li><a href="/articles/">Статьи</a></li><li><a href="/forum/">Форум</a></li><li><a href="/tracker/">Трекер</a></li><li><a href="/search.jsp">Поиск</a></li></ul></nav></header><div style="clear: both"></div>"#)
}
...
fn white2_header(nick: Option<&str>) -> String {
    let login = login_block(nick, false);
    format!(
        r#"<div id="hd"><div id="hdtux"><img src="/img/Tux.svg" height="100%" alt="Linux"></div><a id="sitetitle" href="/">LINUX.ORG.RU</a><ul class="menu"><li id="loginGreating">{login}</li><li><a href="/news/">Новости</a></li><li><a href="/gallery/">Галерея</a></li><li><a href="/articles/">Статьи</a></li><li><a href="/forum/">Форум</a></li><li><a href="/tracker/">Трекер</a></li><li><a href="/search.jsp">Поиск</a></li></ul></div><div style="clear: both"></div>"#,
    )
}
...
fn pony_header(nick: Option<&str>) -> String {
    ...
    format!(
        r#"<div id="hd"><a id="sitetitle" href="/"><img src="/zomg_ponies/img/twilight_logo.png" id="twilight_logo" alt="">PONY.ORG.RU</a><div class="menu"><div id="loginGreating">{login}</div><ul><li><a href="/news/">Новости</a></li><li><a href="/gallery/">Галерея</a></li><li><a href="/articles/">Статьи</a></li><li><a href="/forum/">Форум</a></li><li><a href="/tracker/">Трекер</a></li><li><a href="/search.jsp">Поиск</a></li></ul></div></div><div style="clear: both"></div>"#,
    )
}
...
fn render_footer(public_url: &str, main_page: bool) -> String {
    let info = if main_page {
        ...
        format!(r#"<p id="ft-info"><a href="/about">О Сервере</a> - <a href="/help/rules.md">Правила форума</a><br><a href="https://github.com/maxcom/lorsource/issues">Сообщить об ошибке</a><br><a href="{url}">{url}</a></p>"#)
    };
    let theme_indicator = THEME_INDICATOR.replace(
        "c-0.39,0.39-1.03-0.39-1.41,0",
        "c-0.39-0.39-1.03-0.39-1.41,0",
    );
    format!(r#"<footer id="ft">{info}{theme_indicator}</footer>"#)
}
...
pub async fn apply_theme(State(state): State<AppState>, jar: CookieJar, req: Request, next: Next) -> Response {
    ...
    let is_html = response.headers().get(header::CONTENT_TYPE)
    ...
    text = text.replacen("data-style=\"tango-auto\" data-theme=\"auto\"", &html_attributes, 1);
    text = text.replacen("/tango/combined.css\" data-lor-theme-stylesheet", &format!("{}\" data-lor-theme-stylesheet", view.stylesheet), 1);
    text = text.replacen("<!-- LOR_THEME_HEADER -->", &render_header(&style, nick.as_deref(), main_page), 1);
    text = text.replacen("<!-- LOR_THEME_FOOTER -->", &render_footer(&state.config.public_url, main_page), 1);
...
mod tests {
    ...
    fn every_theme_bundle_is_built() {
        ...
        for style in ["tango", "black", "white2", "waltz", "zomg_ponies"] {
            ...
            let css = std::fs::read_to_string(&path).unwrap_or_else(|error| panic!("{}: {error}", path.display()));
            assert!(css.len() > 55_000, "{} is not an original compiled theme bundle", path.display());
            assert!(css.contains(".swiffy-slider"), "{} misses Swiffy Slider", path.display());
            assert!(css.contains(".tippy-box"), "{} misses Tippy", path.display());
            assert!(css.contains(".hljs"), "{} misses syntax highlighting", path.display());
    ...
    fn profile_style_and_tango_color_mode_are_not_mixed() {
        let base = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/templates/base.html"));
        let settings = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/templates/settings.html"));
