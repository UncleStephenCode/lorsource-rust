...
> Historical snapshot only. The fixed totals below describe the v8 source
> trees and must not be used as current production-readiness evidence. Current
> inventories are regenerated by `scripts/run-compatibility-suite.sh`; route
> equality alone is not behavioral compatibility.
...
See the [current structural report](ROUTE_COVERAGE.md) and generated JSON under
`docs/generated/`; do not carry the v8 totals into current status reports.
