...
Run the authenticated desktop/mobile matrix for all seven themes against an
already seeded instance. Use the canonical `localhost` origin because login
forms and cookies are generated for the configured public URL:

```bash
/tmp/lorsource-browser-venv/bin/python \
  prod_ready_test/theme_browser_matrix.py \
  --base http://localhost:8181
```

This matrix and the userpic/profile lifecycle below fail closed unless
`--base` is exactly an HTTP loopback origin on port `8181`; credentials,
paths, queries, fragments, remote hosts, and other ports are rejected.
...
Run the focused Java-parity lifecycle for userpic upload/removal, avatar
settings, and password-cookie refresh after rebuilding and seeding:

```bash
python3 prod_ready_test/userpic_profile_lifecycle.py \
  --base http://localhost:8181
```

The check logs in as the dedicated `bird49` fixture and exercises the themed
multipart form, Java's empty-file `200`, rejected WebP and animated GIF `400`
responses, accepted static GIF and PNG redirects with `nocache`, exact
`set_userpic`/`reset_userpic` audit metadata, public media bytes, proportional
profile DOM dimensions, persisted `photos`/`avatar` settings, and a refreshed
`remember_me` cookie after a password change. It snapshots the exact user,
settings, user-log and upload-file state in `/tmp`, restores it in `finally`,
and repairs an interrupted previous run before starting, so no random userpic
file or changed fixture credential survives a successful rerun.
...
50 accounts have deterministic local PNG userpics generated by `seed.py`.
Three boundary fixtures are landscape (300×150), portrait (150×300), and
already-small (120×100); the remaining files are 300×300. The HTTP/DOM suite
validates every `/photos/{id}.png` response, proportional 150 px scaling, and
the viewer's `photos=false` suppression contract. It also locks the Java
exceptions to that gate: profiles always render their resolved userpic, and an
active photo remains directly readable even when its owner is blocked.
