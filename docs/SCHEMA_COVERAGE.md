...
> Archived comparison of the old demo dump with the retired Rust development
> migrations. It does not describe the current Java schema and must not be used
> as a migration plan. See `docs/DATABASE_COMPATIBILITY.md`.
...
| rust-only | `email_domains_block` | `` | `auto, block_until, blocked_at, created_at, domain, moderator_id, reason` |
| covered | `groups` | `` | `` |
| covered | `ignore_list` | `` | `` |
