...
pub async fn search(
    ...
    let (items, total, took_ms, section_facet, group_facet, error, prev_link, next_link) =
        if searched {
                ...
                Ok(result) => {
                    let build_link = |o: i64| {
                        ...
                        if range != SearchRange::All {
                            parts.push(format!("range={}", range_id(range)));
                        }
                        if interval != SearchInterval::All {
                            parts.push(format!("interval={}", interval_id(interval)));
                        }
                        if let Some(u) = &q.user {
                            parts.push(format!("user={}", urlencoding::encode(u)));
                        }
                        if usertopic {
                            parts.push("usertopic=true".to_string());
                        }
                        if sort != SearchSort::Relevance {
                            parts.push(format!("sort={}", sort_id(sort)));
                        }
                        if !section.is_empty() {
                            parts.push(format!("section={}", urlencoding::encode(&section)));
                        }
                        if !group.is_empty() {
                    ...
                    let next_link = (next_offset < MAX_OFFSET && result.total > next_offset)
                        .then(|| build_link(next_offset));
                    let prev_link =
                        (offset - SEARCH_ROWS >= 0).then(|| build_link(offset - SEARCH_ROWS));
                    (
                        result.items,
                        result.total,
                        result.took_ms,
                        result.section_facet,
                        result.group_facet,
                        None,
                        prev_link,
                        next_link,
